...
export default function NewWorkCenterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const response = await fetch('/api/v1/work-centers', {
        method: 'POST',
        headers: {
      ...
      addToast({
        type: 'success',
        title: 'Success',
        message: `Work center "${formData.name}" created successfully!`
      })
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-2xl font-bold text-slate-800">Add Work Center</h1>
          </div>
        </div>
        <p className="text-sm text-slate-600 ml-9">
          Create a new work center to organize your production capacity
        </p>
      </div>
